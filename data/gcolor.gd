...
static func get_luminance_10000(color: Color) -> int:
	var r := int(roundf(color.r * 3.0))
	var g := int(roundf(color.g * 3.0))
	var b := int(roundf(color.b * 3.0))
	return (2126 * r + 7152 * g + 722 * b) / 3

static func is_transparent(color: Color) -> bool:
	return is_zero_approx(color.a)

static func closest_opaque(color: Color) -> Color:
...
static func legible_over(background_color: Color) -> Color:
	background_color = closest_opaque(background_color)

	if background_color == CLEAR:
		return WHITE

	var luminance := get_luminance_10000(background_color)

	const MAGIC_THRESHOLD := 4510
	var bright := luminance >= MAGIC_THRESHOLD

	return BLACK if bright else WHITE

static func prv_get_luminance_10000(color: Color) -> int:
	return get_luminance_10000(color)

static func gcolor_is_transparent(color: Color) -> bool:
	return is_transparent(color)

static func gcolor_closest_opaque(color: Color) -> Color:
	return closest_opaque(color)

static func gcolor_legible_over(background_color: Color) -> Color:
	return legible_over(background_color)
